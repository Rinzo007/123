...
def network_from_dict(data: dict) -> Network:
    ...
    for raw in data.get("stops", []):
        network.add_stop(
            Stop(raw["id"], raw["name"], Point(**raw["location"]), raw.get("is_station", False))
        )
    for raw in data.get("vehicle_types", []):
        network.add_vehicle_type(
            VehicleType(
                raw["id"], raw["name"], TransitMode(raw["mode"]),
                raw["capacity"], raw.get("operating_cost_per_km", 0.0)
            )
        )
    ...
    for raw in data.get("routes", []):
        ...
        network.add_route(
            Route(raw["id"], raw["name"], TransitMode(raw["mode"]), tuple(raw["stop_ids"]), line)
        )
    for raw in data.get("services", []):
        network.add_service(
            Service(raw["id"], raw["route_id"], raw["vehicle_type_id"], dict(raw["headway_by_period"]))
        )
    return network


def dumps_network(network: Network) -> str:
    return json.dumps(network_to_dict(network), ensure_ascii=False, indent=2, sort_keys=True)
