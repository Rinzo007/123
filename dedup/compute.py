...
def _metric_unit(metric: str) -> tuple[str, str]:
    ...
    if metric in ("poi", "poi_stops"):
        return "POI у остановок" if metric == "poi_stops" else "POI вдоль маршрута", "шт."
    return "застройка GHS, тыс. м³", "тыс. м³"


def _metric_value(metric: str, raw: float) -> float:
    """Форматирует отчётное значение для метрики."""
    safe = max(0.0, _safe_float(raw))
    if metric in ("poi", "poi_stops"):
        return float(round(safe))
    return round(safe / 1e3, 1)
...
def _metric_display(metric: str, raw: float) -> str:
    ...
    unit = _metric_unit(metric)[1]
    return f"{_metric_value(metric, raw):.1f} {unit}"
...
def _run_greedy_additions(
    ...
    ghs_values: dict[DirectionId, float],
    ctx: _RemovalContext,
    ...
    for rid in order:
        ...
        reason = (
            ...
            f"{_metric_short_label(ctx.metric)}={_metric_display(ctx.metric, ghs_values.get(rid, 0.0))}"
        )
        removed.append(
            _removal_record(
                ctx,
                ...
                rid,
                _VictimPick(
                    rep_id, rep_k, conflicting, ghs_values.get(rid, 0.0)
                ),
                reason,
            )
        )
...
def dedup_compute_additions(
    ...
    """Жадно ДОБАВЛЯЕТ направления по убыванию ценности (GHS/POI), оставляя
    неизбыточные."""
    if not ghs_volumes:
        raise ValueError(
            "ghs_volumes обязателен: дедупликация требует GHS или POI"
        )
    ...
    ghs_values = {
        rid: max(0.0, _safe_float(ghs_volumes.get(rid))) for rid in candidate
    }
