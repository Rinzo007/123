...
def build_xlsx(
    ...
    # Листы пассажиропотока
    if passenger_flow is not None and hasattr(passenger_flow, "route_flows"):
        if passenger_flow.line_results:
            write_lines_kpi_sheet(wb, city, passenger_flow)
        if passenger_flow.route_flows:
            write_routes_flow_sheet(wb, city, passenger_flow)
        if passenger_flow.stop_flows:
            write_stops_flow_sheet(wb, city, passenger_flow)
