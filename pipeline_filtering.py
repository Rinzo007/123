...
def build_filter_limits(config: CliConfig) -> FilterLimits:
    ...
    if radius > 0 and (center_lat is None or center_lon is None):
        ...
        center_lat = None
        center_lon = None
    return FilterLimits(curvilinearity=curvilinearity, min_length_km=min_length, radius_km=radius, center_lat=center_lat, center_lon=center_lon)


def split_active_routes(routes: Sequence[RouteData], *, active_only: bool) -> tuple[list[RouteData], int]:
    if not active_only:
...
def split_ok_and_errors(routes: Sequence[RouteData]) -> tuple[list[RouteData], list[RouteData]]:
    ok = [route for route in routes if not route.error and route.directions]
    bad = [route for route in routes if route.error]
    return ok, bad


def apply_limits(routes: Sequence[RouteData], limits: FilterLimits) -> tuple[list[RouteData], list[tuple[RouteData, str]]]:
    return apply_route_limits(routes, limits)

__all__ = ["apply_limits", "build_filter_limits", "split_active_routes", "split_ok_and_errors"]
