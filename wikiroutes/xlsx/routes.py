...
def routes_headers(*, ghs_stats=None, built_s_stats=None, overture_stats=None, poi_stats=None,
...
def route_direction_row(rd: RouteData, di: int, direction: Any, base_row: list[Any], *,
    ...
    row += _stat_row(rd, di, ghs_dir_stats if ghs_stats else None,
                     attr="volume_m3", scale=1e3)
    row += _stat_row(rd, di, built_s_dir_stats if built_s_stats else None,
                     attr="surface_m2", scale=1e3,
                     fallback=(built_s_stats or {}).get(rd.route_id) if built_s_stats else None)
