...
4. Поездки распределяются по Takt-правилу частотного выбора ``co()/Rr()``:
   для альтернатив используется обратная стоимость ожидания с учётом
   crowding/reliability feedback. Время включает реальное ``cumT`` маршрута, ожидание,
