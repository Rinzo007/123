...
перечисление вариантов поездки (прямые + пересадки), логит-распределение
...
- ``ModeChoiceConfig`` — конкуренция «транзит / авто / пешком»: стоимости
  приводятся к обобщённым минутам, денежные элементы — через VOT;
...
def run_passenger_flow(
    ...
    *,
    ...
    max_transfers : int
        Максимальное число пересадок (0 — без пересадок; поддерживается 0/1).
    headway_min : float | None
        Интервал движения (мин): при задании ожидание на посадке считается
        как ``max(wait_time_min, headway_min / 2)`` вместо ``wait_time_min``.
    wait_crowding_per_100_min : float
        Дополнительное ожидание (мин) на каждый внесённый маршрутом
        пассажиропоток в 100 поездок за первый проход (crowding, >0 включает
        двухпроходный расчёт ожидания).
    transfer_wait_min : float | None
    ...
    mode_choice : ModeChoiceConfig | None
        Конкуренция «транзит / авто / пешком». При ``None`` используется
        ``ModeChoiceConfig()`` (дефолты движка Takt) — выбор режима включён
        по умолчанию; чтобы выключить, передать явную конфигурацию с
        ``car_no_car_share=1.0``, ``two_wheel_share=0.0`` и заведомо
        медленным ``walk_speed_mps``.
    wait_calc : str
