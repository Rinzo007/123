...
def _filter_valid_polygons(arr: np.ndarray) -> tuple[np.ndarray, int]:
    """Оставляет только валидные полигоны/мультиполигоны (типы 3,6), остальные считает невалидными."""
    if arr.size == 0:
        return arr, 0
    removed = 0
    type_ids = shapely.get_type_id(arr)
    ...
    try:
        ...
        # Если векторизованная проверка не удалась, скалярно
...
def _repair_polygonal_geometries(geoms: Any) -> tuple[np.ndarray, int]:
    ...
    try:
        ...
        # Шаг 5: отфильтровать только валидные полигоны/мультиполигоны
        arr, invalid = _filter_valid_polygons(arr)
        removed += invalid
