...
def _try_duckdb_segments(
    ...
    Возвращает постобработанный GDF или ``None``, если ни один провайдер
    не дал данных. Импорт DuckDB и сетевые ошибки логируются, но не
    пробрасываются — вызывающий перейдёт на HTTP-fallback.
    """
    try:
...
def _load_segments_via_http(
    ...
    """HTTP-fallback: STAC-резолвинг ключей, докачка, чтение локальных частей."""
...
def load_overture_segments(
    ...
    Сначала используется официальный DuckDB/S3 путь с пространственным
    pushdown (не требует скачивания целых parquet-part файлов через urllib
    и избегает типичных Windows TLS EOF); при неудаче — HTTP-загрузка
    парт-файлов с STAC-резолвингом.
    """
    try:
    ...
    gdf = _try_duckdb_segments(release, bbox, classes)
    if gdf is not None:
        return gdf

    return _load_segments_via_http(
...
def _iter_chunk_rows(gdf: Any):
    """Итерирует (geometry, id_raw, version_raw) для одного чанка."""
    ...
    yield from zip(geometries, id_values, version_values)
...
def _merge_chunk_into_dedup(
    ...
    *,
    ...
    for geom, raw_id, raw_version in _iter_chunk_rows(gdf):
        if raw_id is None:
